...
public final class Utilities {
  ...
  /*lvyanbing modify add icon */
  public static Bitmap createCompoundBitmap(Bitmap bitmapBg, Bitmap bitmapIcon, float scaleX, float scaleY, float radius)
  {
    int with = bitmapBg.getWidth();
    int height = bitmapBg.getHeight();
    float f1 = sIconTextureWidth / bitmapIcon.getWidth();
    float f2 = sIconTextureHeight / bitmapIcon.getHeight();
    Log.d(TAG, "createCompoundBitmap with is:" + with + "; height is:" + height);
    if ((with < 10) || (height < 10)){
      return scaleBitmap(bitmapIcon, f1, f2);
    }
    Bitmap tempIconBitmap = scaleBitmap(scaleBitmap(bitmapIcon, f1, f2), scaleX, scaleY);
    if (1.0F != radius){
	//tempIconBitmap = getRoundedCornerBitmap(tempIconBitmap, radius);
    }
    Bitmap bitmap = Bitmap.createBitmap(sIconTextureWidth, sIconTextureHeight, Bitmap.Config.ARGB_8888);
    Canvas canvas = new Canvas(bitmap);
    //canvas .drawBitmap(bitmapBg, 0.0F, 0.0F, null);
    canvas .drawBitmap(tempIconBitmap, (sIconTextureWidth - tempIconBitmap.getWidth() >> 1) + 8, (sIconTextureHeight - tempIconBitmap.getHeight() >> 1) + 8, null);
    canvas .drawBitmap(bitmapBg, 0.0F, 0.0F, null);
  ...
  public static Bitmap getRoundedCornerBitmap(Bitmap bitmap, float radius)
  {
    ...
    int k = with >> 1;
    int m = height >> 1;
    float f1 = radius * with;
    float f2 = radius * height;
     if (f1 < f2){
	f2 = f1;
      }
      float f3 = (int)f2 >> 1;
      Log.d(TAG, "getRoundedCornerBitmap");
      RectF rectF = new RectF(k - f3, m - f3, f3 + k, f3 + m);
